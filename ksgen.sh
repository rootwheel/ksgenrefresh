...
if [ -n "$1" -a "$2" = "put" ]
	then
	printf '%s\n' "put $1" "quit" > /tmp/batch.txt
	sshpass -p YOUR_PASS sftp -oBatchMode=no -b /tmp/batch.txt YOUR_SFTPUSER@host.com:/ks
	elif [ -n "$1" -a "$2" = "remove" ]
                then
		printf '%s\n' "rm $1" "quit" > /tmp/batch.txt
               	echo "KS file will be removed in 10 minutes according to"
                at now +10 minute <<< 'sshpass -p YOUR_PASS sftp -oBatchMode=no -b /tmp/batch.txt YOUR_SFTPUSER@host.com:/ks 2>/dev/null'
		else
...
if [[ ("$KEYPRESS" = "6") || ("$KEYPRESS" = "7") || ("$KEYPRESS" = "8") ]]
	then
		deploy ks$KEYPRESS put > /dev/null
		deploy ks$KEYPRESS remove
### Log control ###
#	  	printf '%s\n' "[$(date --rfc-3339=seconds)]: $*logged user $(last -1 | awk 'NR==1 {print $1,$3; exit}') creates a KS$KEYPRESS file for $DSUNAME $IPADDR $ROOTPW " >> ksgen.log
		telegram $DSUNAME "KS$KEYPRESS file with IP: $IPADDR and password: $ROOTPW is ready to install" > /dev/null
	else
